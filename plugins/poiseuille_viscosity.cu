		...
		float shY, float shZ, float Rpipe)
{
	...
	const float4 coo = coosvels[2*pid] + make_float4(0.0f, shY, shZ, 0.0f);
	const float r = sqrt(coo.y*coo.y + coo.z*coo.z);
	const float phi = atan2(coo.z, coo.y) + M_PI;
	...
	atomicAdd(bins+ibin, coosvels[2*pid+1].x);
}
...
class QOS
{
	...
	PinnedBuffer<float> bins;
	PinnedBuffer<int>   binSize;
...
QOS::QOS( MPI_Comm comm, int nRbins, int nPhibins, float n, float force) :

...
void DensityOutletPlugin::setup(Simulation *simulation, const MPI_Comm& comm, const MPI_Comm& interComm)
{
...
void DensityOutletPlugin::beforeCellLists(cudaStream_t stream)
{
    ...
    for (auto pv : pvs)
    {
        ...
        SAFE_KERNEL_LAUNCH(
            DensityOutletPluginKernels::countParticlesInside,
...
double DensityOutletPlugin::computeVolume(long long int nSamples, float seed) const
{
    ...
    SAFE_KERNEL_LAUNCH(
        DensityOutletPluginKernels::countInsideRegion,

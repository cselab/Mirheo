...
__device__ inline bool hasCrossedPlane(float3 pos, float4 plane)
{
    return plane.x * pos.x + plane.y * pos.y + plane.z * pos.z + plane.w >= 0.f;
}

// TODO use cell lists
__global__ void countParticles(PVview view1, float4 plane, int *numberCrossed)
{
    ...
    if (hasCrossedPlane(p.r, plane))
        atomicAdd(numberCrossed, 1);
}

__global__ void moveParticles(PVview view1, PVview view2, float4 plane, int oldsize2, int *numberCrossed)
{
    int pid = blockIdx.x * blockDim.x + threadIdx.x;
    if (pid >= view1.size) return;
...
void ExchangePVSFluxPlanePlugin::beforeParticleDistribution(cudaStream_t stream)
{
    ...
    SAFE_KERNEL_LAUNCH(
            ...
            view1, plane, numberCrossedParticles.devPtr() );
    ...
    SAFE_KERNEL_LAUNCH(
                       ...
                       getNblocks(view1.size, nthreads), nthreads, 0, stream,
                       view1, view2, plane, old_size2, numberCrossedParticles.devPtr() );

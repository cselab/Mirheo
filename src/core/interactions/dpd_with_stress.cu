...
InteractionDPDWithStress::InteractionDPDWithStress(std::string name, const YmrState *state, std::string stressName,
                                                   float rc, float a, float gamma, float kbt, float dt, float power, float stressPeriod) :
    InteractionDPD(name, state, rc, a, gamma, kbt, dt, power, false),
    stressPeriod(stressPeriod)
{
    Pairwise_DPD dpd(rc, a, gamma, kbt, dt, power);
    impl = std::make_unique<InteractionPair_withStress<Pairwise_DPD>> (name, state, stressName, rc, stressPeriod, dpd);
}

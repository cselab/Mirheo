...
VectorField::VectorField(const MirState *state, std::string name, real3 hField, real3 margin) :
    MirSimulationObject(state, name),
    fieldArray_(nullptr),
    margin3_(margin)
{
...
VectorField::VectorField(VectorField&&) = default;

const VectorFieldDeviceHandler& VectorField::handler() const
{
    return *(VectorFieldDeviceHandler*)this;
}

void VectorField::_setupArrayTexture(const float4 *fieldDevPtr)
{
    debug("setting up cuda array and texture object for field '%s'", getCName());

    // Prepare array to be transformed into texture
    auto chDesc = cudaCreateChannelDesc<float4>();
    CUDA_Check( cudaMalloc3DArray(&fieldArray_, &chDesc, make_cudaExtent(resolution_.x, resolution_.y, resolution_.z)) );

    cudaMemcpy3DParms copyParams = {};
    copyParams.srcPtr   = make_cudaPitchedPtr((void*)fieldDevPtr, resolution_.x*sizeof(float4), resolution_.x, resolution_.y);
    copyParams.dstArray = fieldArray_;
    copyParams.extent   = make_cudaExtent(resolution_.x, resolution_.y, resolution_.z);
    copyParams.kind     = cudaMemcpyDeviceToDevice;

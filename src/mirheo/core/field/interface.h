...
/** \brief Driver class used to create a ScalarFieldDeviceHandler.
 */
...
public:
    ...
    /** Prepare the internal state of the \c ScalarField.
...
/** \brief a device-compatible structure that represents a vector field
 */
class VectorFieldDeviceHandler
{
public:
    /** \brief Evaluate the field at a given position
        \param [in] x The position, in local coordinates
        ...
        \rst
        .. warning::
           The position must be inside the subdomain enlarged with a given margin (see \c VectorField)

        \endrst
     */
    __D__ inline real4 operator()(real3 x) const
    {
        //https://en.wikipedia.org/wiki/Trilinear_interpolation
        real4 s000, s001, s010, s011, s100, s101, s110, s111;
        real4 sx00, sx01, sx10, sx11, sxy0, sxy1, sxyz;
        ...
        auto access = [this, &texcoord] (int dx, int dy, int dz)
        {
            const auto val = tex3D<float4>(fieldTex_,
                                           static_cast<float>(texcoord.x + static_cast<real>(dx)),
                                           static_cast<float>(texcoord.y + static_cast<real>(dy)),
                                           static_cast<float>(texcoord.z + static_cast<real>(dz)));
            return make_real4(val);
        };
...
/** \brief Driver class used to create a VectorFieldDeviceHandler.
 */
class VectorField : public VectorFieldDeviceHandler, public MirSimulationObject
{
public:
    /** \brief Construct a \c VectorField object
        \param [in] state The global state of the system
        \param [in] name The name of the field object
        \param [in] h the grid size
        \param [in] margin Additional margin to store in each rank
     */
    VectorField(const MirState *state, std::string name, real3 h, real3 margin);
    virtual ~VectorField();
    ...
    /// \return The handler that can be used on the device
    const VectorFieldDeviceHandler& handler() const;

    /** Prepare the internal state of the \c VectorField.
        Must be called before handler().
        \param [in] comm The cartesian communicator of the domain.
     */
    virtual void setup(const MPI_Comm& comm) = 0;
...
protected:
    ...
    /** \brief copy the given grid data to the internal buffer and create the associated texture object
        \param [in] fieldDevPtr The vector values at each grid point (x is the fast index)
    */
    void _setupArrayTexture(const float4 *fieldDevPtr);
};

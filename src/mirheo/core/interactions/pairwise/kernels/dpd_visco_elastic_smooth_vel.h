...
public:
    ...
    /// constructor
    PairwiseViscoElasticSmoothVelDPDHandler(real rc, real a, real gamma, real power, real H, real n0) :
        ParticleFetcherWithPolChainVectorsAndSmoothVel(rc),
        a_(a),
        gamma_(gamma),
        power_(power),
        invrc_(1.0_r / rc)
                                             ...
                                             const ParticleType src, __UNUSED int srcId) const
    {
        ...
        const real3 duSmooth = dst.smoothVel - src.smoothVel;

        f.dQsrc_dt = beta_ * w * dot(src.Q, dr_r) * duSmooth;
        f.dQdst_dt = beta_ * w * dot(dst.Q, dr_r) * duSmooth;
...
/// Helper class that constructs PairwiseViscoElasticSmoothVelDPDHandler
class PairwiseViscoElasticSmoothVelDPD : public PairwiseKernel, public PairwiseViscoElasticSmoothVelDPDHandler
{
public:

    using HandlerType = PairwiseViscoElasticSmoothVelDPDHandler; ///< handler type corresponding to this object
    using ParamsType = ViscoElasticDPDParams; ///< parameters that are used to create this object

    /// Constructor
    PairwiseViscoElasticSmoothVelDPD(real rc, real a, real gamma, real kBT,
                                     real power, real H, real n0, long seed=42424242) :
        PairwiseViscoElasticSmoothVelDPDHandler(rc, a, gamma, power, H, n0),
        stepGen_(seed),
        kBT_(kBT)
    {}

    /// Generic constructor
    PairwiseViscoElasticSmoothVelDPD(real rc, const ParamsType& p, long seed=42424242) :
        PairwiseViscoElasticSmoothVelDPD(rc, p.a, p.gamma, p.kBT, p.power, p.H, p.n0, seed)
    {}

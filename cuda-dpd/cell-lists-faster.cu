...
#ifdef TEST_MAURO
		int xcid = getCellId(x[j], domainstart.x, invrc, ncells.x);// min(ncells.x - 1, max(0, (int)floor(invrc * (x[j] - domainstart.x))));
#endif
		int ycid = getCellId(y[j], domainstart.y, invrc, ncells.y);// min(ncells.y - 1, max(0, (int)(floor(y[j] - domainstart.y) * invrc)));
		int zcid = getCellId(z[j], domainstart.z, invrc, ncells.z);// min(ncells.z - 1, max(0, (int)(floor(z[j] - domainstart.z) * invrc)));
		...
		float * const xyzuvw, const int bufsize, int * const order, int *loffs)
{
	...
	for(int i = tid; i < count; i += warpSize)
	{
		...
		const int xcid = getCellId(x, domainstart.x, invrc, ncells.x); // min(ncells.x - 1, max(0, (int)floor(invrc * (x - domainstart.x))));
#else
		int id = ids[g];

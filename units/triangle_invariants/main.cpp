...
static real beta(Triangle t, Triangle tref)
{
    ...
    real sign = (v12v13ref * v12v13) > 0.0 ? 1.0 : -1.0;
    // this sign is introduced because of the angle which can be > pi/2
    
    real beta = 0.125 * (e0sq_A0*e1sq_A + e1sq_A0*e0sq_A
                         - 2. * sign *  sqrt((e0sq_A0 * e1sq_A0 - 4.) * (e0sq_A * e1sq_A - 4.))
                         - 8.);
    // print(t);
    // print(tref);
    // printf("beta = %g\n", beta);
...
TEST (TRINAGLE_STRAIN_INVARIANTS, alpha)
{
    ...
    auto t    = genTriangle(4242);
    auto tref = genTriangle(2424);

    real a1, a2, a3;

    a1 = alpha(t, tref); shuffle(t); shuffle(tref);
    a2 = alpha(t, tref); shuffle(t); shuffle(tref);
    a3 = alpha(t, tref);
...
TEST (TRINAGLE_STRAIN_INVARIANTS, beta)
{
    ...
    b1 = beta(t, tref); shuffle(t); shuffle(tref);
    b2 = beta(t, tref); shuffle(t); shuffle(tref);
    b3 = beta(t, tref);
    
    ASSERT_LE(fabs(b1-b2), tolerance);
    ASSERT_LE(fabs(b1-b3), tolerance);
}

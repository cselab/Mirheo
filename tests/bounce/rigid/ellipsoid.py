...
if args.vis:
    ...
    pvEllipsoid = udx.ParticleVectors.RigidEllipsoidVector('ellipsoid', mass=1, object_size=len(coords), semi_axes=args.axes)
...
if args.vis:
    ...
    mdump = udx.Plugins.createDumpMesh("mesh_dump", pvEllipsoid, dumpEvery, path="ply/")
    u.registerPlugins(mdump)


rigStats = udx.Plugins.createDumpObjectStats("rigStats", ov=pvEllipsoid, dump_every=dumpEvery, path="stats")
...
# udx.run --runargs "-n 2"  ../../rigids/createEllipsoid.py $common_args --density 8 --out $f --niter 1000 > /dev/null

...
# cd plugins
# rm -rf force/
# udx.run --runargs "-n 2" python pin_objects.py --velocity 1 0 0 --omega 0 1 0  > /dev/null
...
# cd plugins
# rm -rf force/
# udx.run --runargs "-n 2" python pin_objects.py --velocity 1 0.1 0 --omega 0.1 1 0 --solvent  > /dev/null
# cat stats/*.txt | awk 'NF{NF-=6};1' | sort -n > plugins.out.txt
# cat force/*.txt | sort -n >> plugins.out.txt
...
# cd plugins
# rm -rf force/
# udx.run --runargs "-n 2" python pin_objects.py --velocity 1e10 1e10 0 --omega 1 1e10 1e10 --solvent  > /dev/null
# cat stats/*.txt | awk 'NF{NF-=6};1' | sort -n > plugins.out.txt
# cat force/*.txt | sort -n >> plugins.out.txt

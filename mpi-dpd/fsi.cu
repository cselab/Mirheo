...
ComputeInteractionsFSI::ComputeInteractionsFSI(MPI_Comm _cartcomm) :
firstpost(false), requiredpacksizes(26), packstarts_padded(27)
{
	...
	recvhalos[i].esimate = estimate;
	recvhalos[i].setup(estimate);

	sendhalos[i].estimate = estimate;
	sendhalos[i].setup(estimate);
    }
    ...
    CUDA_CHECK(cudaEventCreateWithFlags(&evPuploaded, cudaEventDisableTiming));
					
    FSI_CORE::Params params = {12.5 , gammadpd, sigmaf};

    CUDA_CHECK(cudaMemcpyToSymbol(FSI_CORE::params, &params, sizeof(KernelsRBC::ParamsFSI)));

    //upload sendbagsizes, scattered_indices
    {
	???
    }
...
void ComputeFSI::pack_p(const Particle * const solute, const int nsolute, cudaStream_t stream)
{
    ...
    FSI_PUP::pack<<< (nsolute + 127 + 32 * 26) / 128, 128, 0, stream >>>(solute, nsolute, packbuffer.data, packbuffer);
...
void ComputeFSI::post_p(cudaStream_t stream)
{
	...
	for(int i = 0; i < 26; ++i)
	    if (send_counts[i] > sendhalos[i].state.capacity)
		packingfailed = true;
	...
	const int npacked = packstarts_padded[26];
	
	CUDA_CHECK(cudaMemcpyAsync(host_packbuffer.data, packbuffer.data, sizeof(Particle) * npacked,
				   cudaMemcpyDeviceToHost, downloadstream));
	
	for(int i = 0; i < 26; ++i)
	    local[i].state.size = local[i].result.size = send_counts[i];
	...
	{
	    MPI_Request reqC;
	    MPI_CHECK( MPI_Isend(send_counts + i, 1, MPI_INTEGER, dstranks[i], TAGBASE_C + i, cartcomm,  &reqC) );
	    reqsendC.push_back(reqC);
	}
	...
	for(int i = 0, c = 0; i < 26; ++i)
	{
	    ...
	    MPI_Request reqP;
	    MPI_CHECK( MPI_Isend(host_packbuffer.data + start, expected * 6, MPI_FLOAT, dstranks[i], TAGBASE_P + i, cartcomm, &reqP) );
	    reqsendP.push_back(reqP);
	    
	    if (count > expected)
	    {
		MPI_Request reqP2;
		MPI_CHECK( MPI_Isend(host_packbuffer.data + start + expected, (count - expected) * 6,
				      ...
				      const Particle * const solute, const int nsolute, Acceleration * accsolute, cudaStream_t stream)
{
	...
	for(int i = 0; i < 26; ++i)
	{
	    ...
	    if (count > expected)
	    {
		...preserve-resize... the accelerations as well...;
	...
	CUDA_CHECK(cudaEventRecord(evAcomputed));
    }

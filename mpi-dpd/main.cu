...
int main(int argc, char ** argv)
{
...

	{
...

		{
			...
			CellProducer* rbcproducer = NULL;
			CellDestroyer* rbckiller  = NULL;

			if (rbcs)
			{
				rbcscoll = new CollectionRBC(cartcomm);
				rbcscoll->setup();
				rbcproducer = new CellProducer(YSIZE_SUBDOMAIN / 10, ZSIZE_SUBDOMAIN / 10, 0, 12, coords[0]);
				rbckiller = new CellDestroyer(60, coords[0]);
			}
			...
			for(it = 0; it < nsteps; ++it)
			{
				...
				if (it % 200 == 0 && rbcscoll)
				{
					rbcproducer->produce(particles, rbcscoll);
					rbckiller->destroy(particles, rbcscoll);
				}

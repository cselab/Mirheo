...
ComputeInteractionsWall::ComputeInteractionsWall(MPI_Comm cartcomm, const int L, Particle* const p, const int n, int& nsurvived):
    cartcomm(cartcomm), L(L), arrSDF(NULL), solid(NULL), solid_size(0), cells(L+2)
{
    printf("hello solid walls\n");

    MPI_CHECK( MPI_Comm_rank(cartcomm, &myrank));
    MPI_CHECK( MPI_Cart_get(cartcomm, 3, dims, periods, coords) );
    
    const float x_cyl = 0.5 * L * dims[0];
    const float y_cyl = 0.5 * L * dims[1];
    const float r_cyl = 0.125 * L * dims[2];
	...
	for(int iy = 0; iy < VPD; ++iy)
		...
		//const float z = (coords[2] * L + iz + 0.5 - 1) * h;

		const float r = sqrt(pow(x - x_cyl, 2) + pow(y - y_cyl, 2));
    ...
    printf("nsurvived is %d -> %.2f%%\n", nsurvived, nsurvived * 100. /n);

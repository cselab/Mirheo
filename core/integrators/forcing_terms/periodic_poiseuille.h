...
public:
	...
	{
		view = create_PVview(pv, pv->local());
	}
	...
	__device__ __forceinline__ float3 operator()(float3 original, Particle p) const
	{
		...
		if (_dir == 0)
			ef.x = gr.y > 0.5f*globalDomainSize.y ? magnitude : -magnitude;

		if (_dir == 1)
			ef.y = gr.z > 0.5f*globalDomainSize.z ? magnitude : -magnitude;

		if (_dir == 2)
			ef.z = gr.x > 0.5f*globalDomainSize.x ? magnitude : -magnitude;
